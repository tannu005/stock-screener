...
const yfImport = require('yahoo-finance2');
...
function getStableRandom(symbol, min, max, seedSuffix) {
    let hash = 0;
...
function convert(quote) {
    ...
    return {
        ...
        analystRating: ['Strong Buy', 'Buy', 'Hold', 'Sell', 'Strong Sell'][Math.floor(getStableRandom(quote.symbol, 0, 4.99, 'rating'))],
        ...
        founded: 0,
        description: '',
        tags: [quote.sector, quote.industry, quote.exchange].filter(Boolean),
        alert: false,
        watchlisted: false,
        candleData: [],
...
async function main() {
    try {
        ...
        const YahooFinanceClass = yfRaw.default || yfRaw;
        const yf = new YahooFinanceClass({ suppressNotices: ['yahooSurvey'] });
        const BATCH_SIZE = 100; // lower batch size
        let allStocks = [];

        for (let i = 0; i < uniqueSymbols.length; i += BATCH_SIZE) {
            const batch = uniqueSymbols.slice(i, i + BATCH_SIZE);
            console.log(`Fetching batch ${Math.floor(i / BATCH_SIZE) + 1} of ${Math.ceil(uniqueSymbols.length / BATCH_SIZE)}...`);
            
            try {
                const quotes = await yf.quote(batch);
                const quoteArr = Array.isArray(quotes) ? quotes : [quotes];
                const mappedStocks = quoteArr.map(convert).filter(s => s !== null);
                allStocks = [...allStocks, ...mappedStocks];
            } catch (err) {
                console.error(`Error fetching batch ${Math.floor(i / BATCH_SIZE) + 1}:`, err.message);
            }
        ...
        const cachePath = path.join(__dirname, '..', 'public', 'market_data_cache.json');
