...
export default function StockCardGroups() {
    const { filteredStocks, setExplorerExpanded, setFilters } = useScreenerStore();
    ...
    useEffect(() => {
        ...
        const ctx = gsap.context(() => {
            gsap.fromTo(sectionRef.current!.querySelectorAll('[data-group]'), 
                { opacity: 0, y: 30 },
                            ...
                            <div className="px-6 py-4 border-t border-white/5 mt-auto">
                                <button
                                    onClick={() => {
                                        ...
                                        // Also apply a preset or sort based on the group
                                        if (group.title === 'Top Gainers') {
                                            setFilters({ changeMin: 0.1 });
                                        } else if (group.title === 'Top Losers') {
                                            setFilters({ changeMax: -0.1 });
                                        }
                                        setTimeout(() => {
                                            document.getElementById('full-data-explorer')?.scrollIntoView({ behavior: 'smooth' });
                                        }, 100);
                                    }}
