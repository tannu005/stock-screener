...
export default function StockTable() {
  const { filteredStocks, selectStock, selectedStock, toggleWatchlist, toggleAlert, watchlist, alerts, setSort, viewMode } = useScreenerStore();
  ...
  const columns = useMemo(() => [
      ...
      cell: ({ row }) => (
        <div className="flex items-center gap-1.5">
          <button
            ...
            style={{ color: watchlist.has(row.original.symbol) ? '#ffd700' : 'rgba(255,255,255,0.4)' }}
          >
            <Star size={12} fill={watchlist.has(row.original.symbol) ? '#ffd700' : 'none'} />
          </button>
          <button
            onClick={e => { e.stopPropagation(); toggleAlert(row.original.symbol); }}
            className="transition-colors hover:scale-110"
            style={{ color: alerts.has(row.original.symbol) ? '#00d4ff' : 'rgba(255,255,255,0.4)' }}
          >
            <Bell size={11} />
          </button>
        </div>
    ...
    helper.accessor('symbol', {
      ...
      cell: info => (
        <div>
          ...
          <div className="font-mono text-white/60 text-xs">{info.row.original.exchange}</div>
        </div>
      ),
    }),
    helper.accessor('name', {
      ...
      header: 'NAME',
      cell: info => (
        <div className="text-xs text-white/80 font-medium truncate max-w-[150px]">{info.getValue()}</div>
      ),
    }),
    helper.accessor('price', {
      size: 100,
      header: viewMode === 'today' ? 'PRICE' : 'PREV CLOSE',
      cell: info => (
        <div className="font-mono font-bold text-white text-sm">
          {formatPrice(viewMode === 'today' ? info.getValue() : info.row.original.prevClose)}
        </div>
      ),
    }),
    helper.accessor('changePct', {
      size: 90,
      header: viewMode === 'today' ? 'CHANGE %' : 'YTD CHANGE %',
      cell: info => {
        const v = viewMode === 'today' ? info.getValue() : info.row.original.week52HighPct * -1; // Simulated historical metric
        const color = v > 0 ? '#00ff88' : v < 0 ? '#ff3366' : '#a3a3a3';
        return (
    ...
    helper.accessor('volume', {
      ...
      header: 'VOLUME',
      cell: info => (
        <div className="font-mono text-xs text-white/90">{formatVolume(info.getValue())}</div>
      ),
    }),
    helper.accessor('volumeRatio', {
      ...
      cell: info => {
        const v = info.getValue();
        const color = v > 2 ? '#ff6b35' : v > 1.5 ? '#ffd700' : 'rgba(255,255,255,0.7)';
    ...
    helper.accessor('marketCap', {
      ...
      header: 'MKT CAP',
      cell: info => (
        <div className="font-mono text-xs text-white/90">{formatMarketCap(info.getValue())}</div>
      ),
    }),
    helper.accessor('peRatio', {
      ...
      cell: info => {
        const v = info.getValue();
        return <div className="font-mono text-xs text-white/80">{v === null ? '—' : v.toFixed(1)}</div>;
      },
    }),
    helper.accessor('rsi', {
      size: 70,
      ...
      cell: info => {
        const v = info.getValue();
        const color = v > 70 ? '#ff3366' : v < 30 ? '#00ff88' : 'rgba(255,255,255,0.8)';
        return (
          <div className="flex items-center gap-1.5">
    ...
    helper.accessor('analystRating', {
      ...
      cell: info => {
        const v = info.getValue();
        const color = RATING_COLORS[v] || '#a3a3a3';
        return (
    ...
    helper.accessor('sector', {
      ...
      header: 'SECTOR',
      cell: info => (
        <div className="text-xs text-white/70 font-mono">{info.getValue()}</div>
      ),
    }),
    helper.display({
      id: 'minichart',
  ...
  ], [formatPrice, formatMarketCap, formatVolume, watchlist, alerts, toggleWatchlist, toggleAlert, viewMode]);
