...
const DEFAULT_SYMBOLS = [
  ...
  'GOOG', 'BA', 'INTC', 'AMD', 'NFLX', 'CSCO', 'PEP', 'COST', 'DELL', 'IBM',
  'ORCL', 'CRM', 'QCOM', 'ADBE', 'TXN', 'AVGO', 'UBER', 'ABNB', 'SNOW', 'PLTR',
  'DIS', 'MCD', 'NKE', 'SBUX', 'KO', 'PG', 'JNJ', 'UNH', 'PFE', 'MRK',
  'XOM', 'CVX', 'T', 'VZ', 'TMUS', 'F', 'GM', 'GE', 'MMM', 'CAT',
  'BAC', 'C', 'WFC', 'GS', 'MS', 'AXP', 'PYPL', 'SQ', 'COIN', 'HOOD',
  'SPY', 'QQQ', 'DIA', 'IWM', 'VTI'
];
...
class StockDataLoader {
  ...
  private async loadHybridStocks(count: number): Promise<Stock[]> {
    try {
      // Fetch live data for popular stocks (up to 100)
      const liveSymbols = DEFAULT_SYMBOLS.slice(0, 100);
      ...
      // Only generate simulated data if fallback is enabled
      const remaining = Math.max(0, count - liveStocks.length);
      if (remaining > 0 && this.fallbackToSimulated) {
        const simulatedStocks = this.loadSimulatedStocks(remaining);
...
export const stockDataLoader = new StockDataLoader({
  mode: (process.env.NEXT_PUBLIC_DATA_MODE as DataSourceMode) || 'hybrid',
